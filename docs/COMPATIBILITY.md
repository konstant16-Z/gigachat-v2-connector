...
| Reasoning | `reasoning_effort`/`thinking` → CoT system prompt | No explicit reasoning field; reasoning likely controlled via model selection (e.g., `GigaChat-3-Ultra` for reasoning) | `MIGRATE` | Remove CoT prompts; rely on model choice — not yet implemented. |
| Structured output | `response_format: {type:"json"|"json_schema"}` (only if no tools) | `model_options.response_format` (same structure) | `SUPPORTED` | Mapper moves `response_format` into `model_options.response_format` (unit-tested `normalized-to-gigachat-v2.test.ts`); live probe for a `json_schema` request not yet run. |
