...
4. **Живая проверка rollback** (`v2:false`) и повторный E2E. Частично сделано:
   черновой V1-прогон (см. следующий пункт) подтвердил маршрут — все запросы шли
   на `api.giga.chat/v1/chat/completions`, ни одного на `/v2`; при
   отзывающемся апстриме V1 функционально работал (сценарии 01/05/06 PASS,
   `factorial` исправлен, `isEven` не тронут, `math.test.ts` создан, `bun test`
   зелёный). Полный smoke-гейт в ночь 18.09 не закрылся из-за
   **нестабильности сети до `api.giga.chat`** (TLS connection reset, пропадал
   DNS; в «зависших» попытках capture видел REQ без RESP — 3/52 против 89/90 в
   рабочем окне), а не из-за дефекта коннектора — повторный прогон нужен при
   стабильном апстриме.
