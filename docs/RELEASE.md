...
| Rollback (offline + live) | регрессия V1, цепочка 02→03→04 | **PASS** (live, 2026-09-19) |
...
Живая проверка rollback — **PASS** (2026-09-19).

Повторный live-прогон цепочки 02→03→04 при `v2:false` закрылся
`02-04:chain PASS(attempt=3)` → `SMOKE RESULT: PASS` при стабильном
апстриме: во всех 9 обработанных сценариях запросы шли только на
`api.giga.chat/v1/chat/completions` (0 на `/v2`), ассерты
factorial/isEven и fixture `bun test` зелёные. Причиной провала ночного
прогона 18.09 был VPN/нестабильность сети до апстрима (TLS connection
reset, пропадал DNS; REQ без RESP 3/52 против 89/90 в рабочем окне),
а не дефект коннектора.
...
4. **Живая проверка rollback** (`v2:false`) и повторный E2E — ✅ **PASS**
   (2026-09-19): цепочка 02→03→04 закрылась `PASS(attempt=3)` при
   стабильном апстриме, маршрут V1 подтверждён во всех сценариях (0 запросов
   на `/v2`). Подробности и причину прошлого провала (VPN/сеть, не дефект
   коннектора) см. в разделе [Rollback](#rollback-37).
5. (справочно) Черновой V1-роллбек-прогон: `/tmp/opencode/rollback-v1-*`,
   harness — `scripts/smoke/run-smoke.sh` с `"v2": false` и V1-проверкой
   маршрута; рабочий конфиг/коннектор/фикстур не менялись.
